...
# Ajout du traitement des informations reçues
...
# Déclarer la LED connectée à GPIO16
led = LED(16)
...
def envoi_test_reponse(chaine_verif, timeout_ms, commande):
    ...
    if chaine_verif in reponse :
        print("La chaine réponse existe : \r\n", reponse)
        # Découper la réponse en lignes
        bloc = (reponse.splitlines(True))
        #print(bloc)
        for line in bloc:
            # Regarder si une ligne contient PORT: 99 qui indique un DOWNLINK
            if line.find("PORT: 99") >= 0:
                # Si la ligne existe
                #print(line[:-1])
                #print("Resultat : ",line[-7:-3])
                # Récupérer les caractères de la commande
                # 00 FF pour 0   et  64 FF pour 1
                resultat = line[-7:-3]
                # Si c'est 0 éteindre la LED
                if resultat == "00FF":
                    led.off()
                # Si c'est 1 allumer la LED
                if resultat == "64FF":
                    led.on()
...
while True :
    ...
    if (is_exist):
        ...
        # Joindre le réseau - rep = 1 si ça fonctionne
            ...
            if (rep):
                # La carte est connectée au réseau mettre le flag indiquant qu'il faut la connecter à False
                is_join = False
            else:
                # Récupérer son ID
                envoi_test_reponse("+ID: AppEui", 1000,"AT+ID")
            ...
            temp = temp_cpu()
            # Valeur envoyée par le dashboard Cayenne
            retour = 0
            # Envoi d'une chaine de data pour test
            # En hexadécimal
            print("temp", temp/10)
            # Conversion au format Cayenne
            payload = ('0200{:02x}0301{:02x}0167{:04x}'.format(capteur,retour, temp))
